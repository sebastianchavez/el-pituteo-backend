...
oneSignalService.sendNotificationChangePassword = (toId) => {
    return new Promise(async (resolve, reject) => {
        const { title, message } = pushTemplate.changePassword()
        const key = `Basic ${ONESIGNAL_APIRESP}`
        const body = {
            "app_id": ONESIGNAL_APP_ID,
            "data": {
...
oneSignalService.sendNotificationFromAdmin = (obj, toId) => {
    return new Promise(async (resolve, reject) => {
        const { title, message } = pushTemplate.notification(obj)
        const key = `Basic ${ONESIGNAL_APIRESP}`
        const body = {
            "app_id": ONESIGNAL_APP_ID,
            "data": {
